...
export default function CopySettings() {
  ...
  const [editingContent, setEditingContent] = useState<CopyContentData | null>(
    null,
  );
    ...
    if (
      (content.page === "contact" &&
        (content.section === "business-hours" ||
          content.section.includes("services-"))) ||
      (content.page === "home" && content.section === "about-us") ||
      content.section === "our-approach" ||
      (content.page === "about" &&
        (content.section === "maureen-bio" ||
          content.section === "joanna-bio" ||
          content.section === "meet-team-header" ||
          content.section === "team"))
    ) {
      const relatedSections = copyContent.filter(
        (item) =>
  ...
  const filteredContent = copyContent.filter((item) => {
    // Show our-approach records in home page tab even though they're stored under 'about'
    if (selectedPage === "home" && item.section === "our-approach") {
      return true;
    }
    // Hide our-approach records from about page tab since they now appear in home
    if (selectedPage === "about" && item.section === "our-approach") {
      return false;
    }
    return item.page === selectedPage;
  });
  ...
  const getGroupedContent = (content: CopyContentData[]) => {
    ...
    return Object.entries(sectionGroups).map(([section, items]) => {
      ...
      const getSectionDescription = (section: string, itemCount: number) => {
        switch (section) {
          case "about-us":
            return "Main about us content and description";
          case "team":
            return "Team introduction and overview";
          case "our-approach":
            return "Company approach and methodology";
          case "meet-team":
            return "Individual team member information";
          case "maureen-bio":
            return "Maureen's complete biography and background";
          case "joanna-bio":
            return "Joanna's complete biography and background";
          case "business-hours":
            return "Complete business hours information";
          case "services-residential":
            return "Residential services details";
          case "services-commercial":
            return "Commercial services details";
          default:
            return `${itemCount} content ${itemCount === 1 ? "item" : "items"}`;
        }
      };
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <CardTitle className="text-lg">
                            {item.type === "group"
                              ? item.title
                              : (item.data as CopyContentData).title ||
                                (item.data as CopyContentData).sectionKey}
                          </CardTitle>
                          <CardDescription>
                            {item.type === "group"
                              ? item.description
                              : (item.data as CopyContentData).description ||
                                `${(item.data as CopyContentData).section} - ${(item.data as CopyContentData).contentType}`}
                          </CardDescription>
                        </div>
                          ...
                          {item.type === "group" ? (
                            <>
                              ...
                              <Badge variant="outline">
                                {(item.data as CopyContentData[]).length}{" "}
                                {(item.data as CopyContentData[]).length === 1
                                  ? "item"
                                  : "items"}
                              </Badge>
                            </>
                            ...
                            <>
                              <Badge variant="secondary">
                                {(item.data as CopyContentData).section}
                              </Badge>
                              <Badge variant="outline">
                                {(item.data as CopyContentData).contentType}
                              </Badge>
                            </>
                          )}
                        </div>
                        ...
                        {item.type === "group" ? (
                          ...
                          <div>
                            ...
                            <div className="mt-1 space-y-2">
                              {(item.data as CopyContentData[]).map(
                                (content) => (
                                  <div
                                    key={content.id}
                          ...
                          <div>
                            ...
                            <div
                              ...
                              style={{ color: "var(--color-text)" }}
                              dangerouslySetInnerHTML={{
                                __html: (item.data as CopyContentData).content,
                              }}
                            />
                          </div>
                        )}
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() =>
                              openEditDialog(
                                item.type === "group"
                                  ? (item.data as CopyContentData[])[0]
                                  : (item.data as CopyContentData),
                              )
                            }
                          >
            ...
            <DialogDescription>
              {editingSections.length > 0
                ? `Update all content for ${
                    editingSections[0]?.section === "business-hours"
                      ? "Business Hours"
                      : editingSections[0]?.section.includes("services-")
                        ? "Services"
                        : editingSections[0]?.section === "about-us"
                          ? "About Us"
                          : editingSections[0]?.section === "our-approach"
                            ? "Our Approach"
                            : editingSections[0]?.section === "maureen-bio"
                              ? "Maureen Bio"
                              : editingSections[0]?.section === "joanna-bio"
                                ? "Joanna Bio"
                                : editingSections[0]?.section ===
                                    "meet-team-header"
                                  ? "Meet the Team Header"
                                  : editingSections[0]?.section === "team"
                                    ? "Team"
                                    : editingSections[0]?.section
                  } section`
